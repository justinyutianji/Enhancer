...
class ExplaiNN3_interact(nn.Module):
    def __init__(self, num_cnns, input_length, num_classes, 
                 filter_size=19, num_fc=2, pool_size=7, pool_stride=7, 
                 drop_out=0.3, weight_path=None):
        super(ExplaiNN3_interact, self).__init__()
        self._options = {
            "num_cnns": num_cnns,
            "input_length": input_length,
            "num_classes": num_classes,
        ...
        activation_length = int(((input_length - (filter_size-1)) - (pool_size-1)-1)/pool_stride + 1)
        self.interaction = nn.Sequential(
            nn.Conv1d(in_channels= 2 * num_interaction_term,
               out_channels=num_interaction_term, kernel_size=filter_size, groups= num_interaction_term),
            nn.BatchNorm1d(num_interaction_term),
            ExpActivation(),
            nn.MaxPool1d(pool_size, pool_stride),
            nn.Flatten(),
            Unsqueeze(),
            nn.Conv1d(in_channels=int(((activation_length - (filter_size-1)) - (pool_size-1)-1)/pool_stride + 1) * num_interaction_term,
