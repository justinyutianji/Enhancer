...
seeds = [random.randint(1, 1000) for _ in range(2)]
batches = [96,168]
    ...
    for batch in batches:
        for cnn in num_cnns:
            ...
            for learning_rate in lrs:
                ...
                os.makedirs(f'/pmglocal/ty2514/Enhancer/Enhancer/results/ExplaiNN3_cnn{cnn}_dp{dropout}_ba{batch}_lr{formatted_lr}_seed{seed}', exist_ok=True)
                pathway = f'/pmglocal/ty2514/Enhancer/Enhancer/results/ExplaiNN3_cnn{cnn}_dp{dropout}_ba{batch}_lr{formatted_lr}_seed{seed}/models'
                ...
                # Generating Plots
                pathway = f'/pmglocal/ty2514/Enhancer/Enhancer/results/ExplaiNN3_cnn{cnn}_dp{dropout}_ba{batch}_lr{formatted_lr}_seed{seed}/best_pearson'
                model_path = f'/pmglocal/ty2514/Enhancer/Enhancer/results/ExplaiNN3_cnn{cnn}_dp{dropout}_ba{batch}_lr{formatted_lr}_seed{seed}/models/model_epoch_{best_pearson_epoch}.pth'
                ...
                pathway = f'/pmglocal/ty2514/Enhancer/Enhancer/results/ExplaiNN3_cnn{cnn}_dp{dropout}_ba{batch}_lr{formatted_lr}_seed{seed}/best_r_square'
                model_path = f'/pmglocal/ty2514/Enhancer/Enhancer/results/ExplaiNN3_cnn{cnn}_dp{dropout}_ba{batch}_lr{formatted_lr}_seed{seed}/models/model_epoch_{best_r2_epoch}.pth'
                mse, rmse, mae, r2, pearson_corr, spearman_corr = regression_model_plot(
                    input_model, test_loader, train_losses_by_batch, test_losses_by_batch, 
                    device, results, label_mode = "both",
                    save_plot = True, dir_path = pathway, model_path = model_path)
                print("Deleting Models")
                if os.path.exists(f'/pmglocal/ty2514/Enhancer/Enhancer/results/ExplaiNN3_cnn{cnn}_dp{dropout}_ba{batch}_lr{formatted_lr}_seed{seed}/models'):
                    # Remove the directory and all its contents
                    shutil.rmtree(f'/pmglocal/ty2514/Enhancer/Enhancer/results/ExplaiNN3_cnn{cnn}_dp{dropout}_ba{batch}_lr{formatted_lr}_seed{seed}/models')
